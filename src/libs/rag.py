...
from libs.config import env, provider
...
def get_embeddings():
    ...
    if p == "openai":
        from libs.llm.openai import get_embeddings as get_openai_embeddings

        return get_openai_embeddings()

    if p == "gemini":
        from libs.llm.gemini import get_embeddings as get_gemini_embeddings
...
def get_openai_llm():
    from libs.llm.openai import get_llm as get_openai_llm_impl

    return get_openai_llm_impl()


def get_gemini_llm():
    from libs.llm.gemini import get_llm as get_gemini_llm_impl
